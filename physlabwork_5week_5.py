...
#read data from ods
all_data = pd.read_excel('physlabwork_5week_3.ods')
x_data = all_data['length, m'].dropna().to_numpy()
x_name = "length, m"
x_error = all_data['error length'].dropna().to_numpy()
y_data = all_data['delta pressure, Pa'].dropna().to_numpy()
y_name = "delta pressure, Pa"
y_error = all_data['error delta pressure'].dropna().to_numpy()
...
axes.set_title("flow = 0.1083 l/s, diameter = 5.2 mm");
...
#save
plt.savefig('physlabwork_5week_plot_5.svg')
