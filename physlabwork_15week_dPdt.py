...
x_data = all_data["t, s (start)"].dropna().to_numpy()
x_name = "t, s"
x_error = all_data["t, s (start) (error)"].dropna().to_numpy()
#all_data[""].dropna().to_numpy()
y_data = all_data["P, mmHg*10^(-4) (start)"].dropna().to_numpy()
y_name = "P, mmHg*10^(-4)"
y_error = all_data["P, mmHg*10^(-4) (start) (error)"].dropna().to_numpy()
#all_data[""].dropna().to_numpy()
...
text='fitting line, y = ('+str(round (popt[0], 7))+" +- "+str(error_a)+') * x + ('+str(round (popt[1], 7))+" +- "+str(error_b)+')'
axes.legend(['exp.data', text])

axes.errorbar(x_data, y_data, xerr=x_error, yerr=y_error, color='black', fmt='o', capsize=5)
#axes.errorbar(x_data, y_data, xerr=x_error, yerr=y_error, ecolor='black')
#plt.errorbar(x, y, yerr=errors, fmt='o', capsize=0)
