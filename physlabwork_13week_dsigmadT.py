...
#read data from ods
num = 0
name = "physlabwork_13week"
#"physlabwork_6week_"+str(num)
all_data = pd.read_excel(name+".ods")
x_data = all_data["T, C"].dropna().to_numpy()
x_name = "T, C"
x_error = all_data["T, C (error)"].dropna().to_numpy()
#all_data[""].dropna().to_numpy()
y_data = all_data["/sigma, mN/m"].dropna().to_numpy()
y_name = "$\\sigma$, mN/m"
y_error = all_data["/sigma, mN/m (error)"].dropna().to_numpy()
#all_data[""].dropna().to_numpy()
...
text='l.sq.fit.line, y = ('+str(round (popt[0], 7))+" +- "+str(error_a)+') * x + ('+str(round (popt[1], 7))+" +- "+str(error_b)+')'
...
#save
if save:
	plt.savefig("physlabwork_13week.png")
